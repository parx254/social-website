...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($username_err) && empty($firstname_err) && empty($lastname_err) && empty($email_err) && empty($password_err) && empty($confirm_password_err)) {
        ...
        if ($stmt = mysqli_prepare($con, $sql)) {
            ...
            // Attempt to execute the prepared statement
            if (mysqli_stmt_execute($stmt)) {
                // Prepare insert statement for profiles table
                ...
                if ($stmt2 = mysqli_prepare($con, $sql2)) {
                    ...
                    // Close statement for profiles table
            ...
            // Close statement for users table
            mysqli_stmt_close($stmt);
        } else {
            echo "Something went wrong. Please try again later.";
        }
    } else {
        echo "Input validation errors."; // Handle input errors here
    }
